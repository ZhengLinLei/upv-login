...
def create_mosaic(image_folder, output_path):
    ...
    try:
        # Open images in order and determine the maximum width and height, but before if the image is not empty
        images = [(Image.open(f).resize((240, 280)), f) for f in image_files if os.path.getsize(f) > 0]
        # Sort by filename
